 ...
 * 
 * @author Tiger Zhao; edited by Ian Choi, Ryan Jian, Ishan Jha, Yucheng Chen
...
public class Game extends World
{
    ...
    private static GreenfootSound boom = new GreenfootSound("boom.mp3");
    private static GreenfootSound boom2 = new GreenfootSound("boom2.mp3");
    private static GreenfootSound sirens = new GreenfootSound("airRaid.mp3");
    private static GreenfootSound bigBoom = new GreenfootSound("nuke.mp3");
    ...
    private int[][] blockO = new int[][]{{1,1},
            {1,1}}; 
    private int[][] blockB = new int[][] {{1}};
    private int[][] blockN = new int[][] {{1}};
    ...
    private boolean zeroPlayed = false;
    
    private boolean nuking = false;
    private int nukeCounter = 0;
    private int nukeDuration = 300; // how many frames the animation lasts
    
    private int bombPauseDelay = 0;    // current pause counter
    private int maxBombPause = 20; 
    ...
    public void act(){
        ...
        if (nuking) {
            nukeCounter++;
            if (nukeCounter >= nukeDuration) {

                Greenfoot.setWorld(new WinScreen(score, level, totalLinesCleared));
            }
    ...
    public void checkLose() {
        for (int x = 4; x <= 6; x++) {
            Block b = grid[1][x];
            if (b != null && (!b.equals("B") || !b.equals("N"))) { // ignore bombs
    ...
    public void setInBlock(){
        ...
        if (curBlockType.equals("B")) {
            ...
            boom2.play();
            explodeBomb();
            bombPauseDelay = maxBombPause;
        }
        if (curBlockType.equals("N")) {
            bigBoom.play();
            wipeGrid();
            nuking = true;
            nukeCounter = 0;
    ...
    public void drawScreen(){
        ...
        if(curLinesCleared >= 6){
            level = Math.min(level + 1, 3);
            curLinesCleared = 6 - curLinesCleared;
            score += level * 100;
            
            if (level != 2)
                showLevelCutscene(level);
    ...
    public void generateBlocks(){
        ...
        int r = random.nextInt(10); //random number genration
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                ...
                if (ny >= 0 && ny < 20 && nx >= 0 && nx < 10) {
                    ...
                    addObject(new BombAnimation("B"), nx * 30 + 50, ny * 30 + 50);
                }
            }
        }
    }
    ...
    public void wipeGrid() {
        ...
        removeObjects(getObjects(Block.class)); // clear screen
    
        for (int i = 0; i < grid.length; i++) {
            for (int j = 0; j < grid[0].length; j++) {
                grid[i][j] = null;
                addObject(new BombAnimation("N"), j * 30 + 50, i * 30 + 50);
            }
        }
    }
    
    public void showLevelCutscene(int level) {
        String filename = "level" + level + ".png"; // level1.png, level2.png...
        GreenfootImage img = new GreenfootImage(filename);
    
        CutsceneActor cs = new CutsceneActor(img);
    
        addObject(cs, getWidth()/2, getHeight()/2);
    
        Greenfoot.delay(100);  // pauses the whole game
